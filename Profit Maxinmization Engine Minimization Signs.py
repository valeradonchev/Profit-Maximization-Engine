...
RPP = 100 + (700 * DCLS) # RULER_POLITICAL_POWER
SPP = 100 + (100 * DCLS) # SPECIALIST_POLITICAL_POWER
WPP = 100 + (0) # WORKER_POLITICAL_POWER
...
def Food(x):
    ...
    food = [PFI-FFOA, PFI, PFI, PFI, PFI, PFI, PFI,
            0, 0, 0, 0, 0 ,0]
    return food @ x
...
def Amenities_used(x):
    ...
    amenities_used = np.array([PAI, PAI, PAI, PAI, PAI, PAI, PAI,
                               0, 0, 0, 0, 0, 0])
...
def Consumer(x):
    ...
    consumer = [WCI, WCI, WCI, WCI, SCI+ECI, SCI-ACOA, SCI,
                0, 0, 0, 0, 0, 0]
    return consumer @ x
...
def Energy(x):
    ...
    energy = [-WTO, -WTO, -WTO-TEOA, -WTO-CTO, -STO, -STO, -STO,
              FEU, MEU, GEU, TEU, IEU, CEU]
    return energy @ x
...
# print("Amenities: " + str(Amenities(solution.x)))
print(-solution.fun)